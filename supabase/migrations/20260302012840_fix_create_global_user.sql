-- =====================================================
-- Update create_global_user RPC for Admin CMS
-- to prevent inviting existing store members
-- =====================================================
...
BEGIN
    ...
    IF v_existing_user_id IS NOT NULL THEN
        -- CRITICAL COMPONENT: Prevent inviting users who are already staff/owners in another store
        IF EXISTS (SELECT 1 FROM public.store_members WHERE user_id = v_existing_user_id) THEN
            RETURN jsonb_build_object('success', false, 'message', 'Akun ini sudah terdaftar sebagai kasir atau pemilik toko lain dan tidak dapat diundang menjadi pemilik baru lewat fitur ini.');
        END IF;

        -- If user exists but has no store (orphan account) and we are forcing a password change
        ...
        RETURN jsonb_build_object(
            'success', true,
            'message', 'User terdaftar di sistem tapi belum memiliki akses toko. Password diperbarui.',
